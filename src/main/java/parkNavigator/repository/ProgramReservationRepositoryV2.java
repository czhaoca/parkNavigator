...
public class ProgramReservationRepositoryV2 implements ProgramReservationRepository {
  ...
  @Override
  public List<ProgramInfo> findInfoByEmail(String email) {
    String sql = "SELECT * FROM program_info pi " +
        "WHERE NOT EXISTS " +
        "(select distinct (p.program_id) from program_reservation p " +
        "WHERE NOT EXISTS " +
        "(SELECT p2.program_id from program_reservation p2 where p2.email=:email and p2.program_id=pi.id))";

    SqlParameterSource param = new MapSqlParameterSource()
        .addValue("email", email);
    List<ProgramInfo> updated = template.query(sql, param, programInfoRowMapper());
    return updated;
  }
