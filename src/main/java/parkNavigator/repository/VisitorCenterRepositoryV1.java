...
import parkNavigator.Mapper.VisitorCenterMapper;
import parkNavigator.domain.VisitorCenter;
...
public class VisitorCenterRepositoryV1 {
  ...
  public VisitorCenter getById(int id) {
    return (VisitorCenter) jdbcTemplate.queryForObject("select * from Visitor_Center where id = ?",
        new VisitorCenterMapper(), id);
  }

  public VisitorCenter getByParkId(int parkId) {
    return (VisitorCenter) jdbcTemplate.queryForObject("select * from Visitor_Center where park_id = ?",
        new VisitorCenterMapper(), parkId);
  }
