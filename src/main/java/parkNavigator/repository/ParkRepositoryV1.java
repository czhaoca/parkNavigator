...
import parkNavigator.Mapper.ParkMapper;
...
public class ParkRepositoryV1 {
  ...
  public int addPark(Parks p) {
    ...
    count = jdbcTemplate.update(query, p.getId(), p.getProvinceId(), p.getParkName(), p.getParkAddress(),
        p.getOpenHour(), p.getCloseHour());
    return count;
  }
